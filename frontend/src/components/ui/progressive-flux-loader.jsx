...
const DEFAULT_PHASES = [
  { at: 0,   label: "initializing search" },
  { at: 15,  label: "parsing job description" },
  { at: 30,  label: "generating embeddings" },
  { at: 50,  label: "scoring 100k candidates" },
  { at: 70,  label: "applying filters" },
  { at: 85,  label: "ranking results" },
  { at: 95,  label: "generating insights" },
  { at: 100, label: "candidates ready" },
];
...
function FluxLabel({ label, reduced, className }) {
  const base = cn(
    "absolute inset-0 flex items-center justify-center text-center text-2xl font-semibold tracking-tight text-white/70 sm:text-3xl",
    className,
  );
...
export function ProgressiveFluxLoader({
  ...
  return (
    <div className={cn("mx-auto flex w-full max-w-md flex-col items-center gap-8", className)}>
      ...
      <div
        className={cn(
          "relative h-5 w-full overflow-hidden rounded-full shadow-[inset_0_2px_3px_rgba(0,0,0,0.45),inset_0_-1px_2px_rgba(255,255,255,0.05)]",
          "bg-white/10",
          barClassName,
        )}
        role="progressbar"
